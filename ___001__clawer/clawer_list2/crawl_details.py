...
def format_text_content(text):
    ...
    title_keywords = ['的药方', '的功效', '出处', '组成', '解释', '主治', '用法', '功效', '方解', '注意事项', '禁忌', '附方',
                     '性味', '归经', '功能', '主治', '用法用量', '注意事项', '化学成分', '药理作用', '临床应用', '配伍', '鉴别']
...
def crawl_herb_details():
    """
    遍历中药大全列表，爬取每个中药的详情并保存为txt文件
    """
    # 读取Excel文件
    excel_file = os.path.join(os.path.dirname(__file__), '中药大全列表.xlsx')
    ...
    print(f"共找到 {len(df)} 个中药")
    
    # 创建保存目录
    output_dir = os.path.join(os.path.dirname(__file__), '中药详情')
    ...
    with tqdm(total=len(df), desc="爬取进度", unit="个") as pbar:
        ...
        for index, row in df.iterrows():
            herb_name = str(row['中药名称']).strip()
            herb_url = str(row['链接']).strip()
            
            # 清理文件名
            safe_filename = sanitize_filename(herb_name)
            txt_file = os.path.join(output_dir, f"{safe_filename}.txt")
            
            # 更新进度条描述
            pbar.set_description(f"正在处理: {herb_name[:20]}")
            ...
            try:
                # 发送请求
                response = requests.get(herb_url, headers=headers, timeout=30)
                response.encoding = 'utf-8'
                
                if response.status_code != 200:
                    fail_count += 1
                    failed_herbs.append((herb_name, herb_url, f"状态码: {response.status_code}"))
                ...
                if not text_content or len(text_content.strip()) < 10:
                    fail_count += 1
                    failed_herbs.append((herb_name, herb_url, "内容为空"))
                ...
                with open(txt_file, 'w', encoding='utf-8') as f:
                    f.write(f"中药名称: {herb_name}\n")
                    f.write(f"链接: {herb_url}\n")
                    f.write("=" * 80 + "\n\n")
                ...
                fail_count += 1
                failed_herbs.append((herb_name, herb_url, str(e)))
    ...
    if failed_herbs:
        print(f"\n失败的中药列表:")
        for name, url, error in failed_herbs[:10]:  # 只显示前10个
            print(f"  - {name}: {error}")
        if len(failed_herbs) > 10:
            print(f"  ... 还有 {len(failed_herbs) - 10} 个失败项")
        ...
        with open(fail_file, 'w', encoding='utf-8') as f:
            f.write("失败的中药列表\n")
            f.write("=" * 80 + "\n\n")
            for name, url, error in failed_herbs:
                f.write(f"中药名称: {name}\n")
                f.write(f"链接: {url}\n")
                f.write(f"错误: {error}\n")
                f.write("-" * 80 + "\n")
